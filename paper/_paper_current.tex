...
\begin{table*}[t!]
  \centering
  \caption{Comparison of schema evolution and migration operations across existing approaches and SMEL.$^\ast$}
  \label{tab:operation-comparison}
  \resizebox{\textwidth}{!}{
  ...
  \begin{flushleft}
  \footnotesize
  $^\ast$ SMEL supports two syntax variants: \emph{specific} (e.g., \texttt{ADD\_PROPERTY}) and \emph{general} (e.g., \texttt{ADD PROPERTY}).\\
...
\begin{figure}[t]
    ...
    \node[lbl, anchor=north west] at (-7.2,-14.2) {
      ...
      \textcolor{blue}{Blue} = SMEL extension
    };

    \end{tikzpicture}
    }% end resizebox
    \caption{M-Model\textsuperscript{+}: M-Model (black) with SMEL extensions ({\textcolor{blue}{blue}})}
...
\draw[arr] (v2)  -- (fe);
\draw[arr] (fe)  -- (exp);
\node[file] at (6.6, 1.2) (script) {\textsc{Smile} Script (.smel/.smel\_gen)};
\draw[arr] (script) -- (sml);
%% Layer 1
